...
drawHeader($session, array(), array("commutes.css", "checkout.css", "table.css"), false);
?>
<?php function drawItems(PDO $db, array $cartList)
{ ?>

  <h2>Your Order</h2>
  <div class="orders">
    <section class="table recent">
      <h2 class="th tr">
      ...
      foreach ($cartList as $entry) {
        ...
        $id = $arr[0];
      ?>
        <?php $prod = Product::getProduct($db, $id);
        $restName = Product::getProductRestaurantName($db, $id);
        $total = $total + ($prod->price * $quantity);
        ?>

        <span class="tr">
          <span class="td"><?= $prod->name ?></span>
          <span class="td"><?= $restName ?></span>
          <span class="td"><?= $quantity ?></span>
          <span class="td"><?= number_format($prod->price * $quantity, 2) ?> €</span>
          <span class="td">
            <div id="foodPic">
      ...
      <?php } ?>
      <span class="tr" id="end">
        <span class="td"></span>
        <span class="td"></span>
        <span class="td">Total</span>
        <span class="td"><?= number_format($total, 2) ?>€</span>
        <span class="td">
          <form class="btn" action="../actions/action_checkout.php" method="post" enctype="multipart/form-data">
...
<?php function drawMaps(PDO $db, array $cartList, Session $session)
{ ?>
  ...
  <script src="../js/commutes.js"></script>
  <script>
    const restAddress = "<?php echo Product::getProductRestaurantAddress($db, explode(":", $cartList[0])[0]); ?>";
    const deliveryAddress = "<?php echo User::getAddress($db, $session->getId()) ?>";

    async function getCoordinates(address) {
      const response1 = await fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + address + '&key=' + 'AIzaSyBpO0y-0Z5hnrvBKeYpMbMXeX8qDxuw5ug')
    ...
    async function getDestCoordinates(address) {
      const response1 = await fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + address + '&key=' + 'AIzaSyBpO0y-0Z5hnrvBKeYpMbMXeX8qDxuw5ug')
      const response = await response1.json();
    ...
    async function initMap() {
      var rest = await getCoordinates(restAddress);
      var del = await getDestCoordinates(deliveryAddress);
      console.log(del);
      const CONFIGURATION = {
        "defaultTravelMode": "DRIVING",
        "isMetric": true,
        "initialDestinations": [{
          "placeId": del.place_id,
          "geometry": del.geometry
        }],
        "mapOptions": {
          "center": {
            "lat": rest.lat,
        ...
        "mapsApiKey": "AIzaSyBpO0y-0Z5hnrvBKeYpMbMXeX8qDxuw5ug"
      };
      new Commutes(await CONFIGURATION);
    }
  </script>
  <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBpO0y-0Z5hnrvBKeYpMbMXeX8qDxuw5ug&callback=initMap&libraries=places,geometry&solution_channel=GMP_QB_commutes_v1_c" async defer></script>
<?php } ?>
...
<main>
  <div class=wrapper1>
    <div class="profile">
      <div id="pic">
        <div id="personPic">
          <img src=<?= User::getImage($user->id) ?>>
          <div id="shortInfo">
            <p> <?= $session->getName() ?></p>
            <p> <?= $session->phoneNumber ?></p>
          </div>
        </div>
        <?php drawMaps($db, $cartList, $session) ?>
      </div>
    </div>
    <div class="order">
      <?php drawItems($db, $cartList) ?>
    </div>
  </div>
